...
export function RetailersSection({ product }: { product: Product }) {
  ...
  const fmt = (v: number) =>
    new Intl.NumberFormat("en-US", { style: "currency", currency: product.price.currency, maximumFractionDigits: 0 }).format(v);
    ...
    <div className="rounded-[22px] border border-border bg-surface p-5 sm:p-6" style={{ boxShadow: "var(--card-shadow)" }}>
      ...
      <div className="mt-1">
        {listings.map((listing, i) => {
          const diff = listing.price - cheapest;
          return (
            <div key={listing.name} className="border-t border-border py-3 first:border-t-0 first:pt-2">
              <div className="flex items-start justify-between gap-3">
                <button
                  type="button"
                  onClick={() => showToast("Retailer links aren't available yet")}
                  className="pressable mt-0.5 flex items-center gap-1.5 text-left text-sm font-semibold text-link hover:underline"
                >
                  {listing.name}
                ...
                <div className="shrink-0 text-right">
                  <ExpandablePrice
                    ...
                    source={listing.name}
                    rows={[
                      { label: "Listed price", value: fmt(listing.price) },
                      {
                        label: "vs best price",
                        value: diff === 0 ? "Cheapest available" : `${fmt(diff)} more`,
                        tone: diff === 0 ? "buy" : "dont",
                      },
                      { label: "vs typical", value: fmt(product.price.typical), meta: "typical" },
                    ]}
                    note="Illustrative demo listing — not a real offer from a real retailer."
                  />
                </div>
              </div>
            </div>
